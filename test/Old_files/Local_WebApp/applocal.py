...
contract_address = "0xd4c601e21852c6D9E03e3Fb004BC88A740B3bb3f"  # Replace with your contract address
abi = [
    {
        "inputs": [
            {
...
def retrieve_cid_from_blockchain(file_id):
    ...
    try:
        ...
        print(f"CID: {cid}, File Path: {file_path}, Uploader: {uploader}")
        return cid
...
def search_multi_keywords_with_file_index_and_blockchain(keywords):
    ...
    if keyword_id_sets:
        common_ids = set.intersection(*keyword_id_sets)
        results["Common"] = {
            id_: retrieve_cid_from_blockchain(int(id_))  # Retrieve CID for common IDs
            for id_ in common_ids
        }
    ...
    for keyword_result in keyword_results.items():
        branch, ids = keyword_result
        results[branch] = {
            id_: retrieve_cid_from_blockchain(int(id_))  # Retrieve CID for each ID
            for id_ in ids
        }
